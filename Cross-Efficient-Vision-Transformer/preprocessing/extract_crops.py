...
from project_utils import get_video_paths, get_method, get_method_from_name
    ...
    try:
        ...
        for i in range(frames_num):
            ...
            for j, crop in enumerate(crops):
                cv2.imwrite(
                    os.path.join(opt.output_path, id, "{}_{}.png".format(i, j)), crop
                )
        if counter == 0:
            print(video, counter)
    except e:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_path", default="", type=str, help="Videos directory")
    ...
    with Pool(processes=cpu_count() - 2) as p:
        with tqdm(total=len(paths)) as pbar:
            for v in p.imap_unordered(
                partial(extract_video, root_dir=opt.data_path, dataset=dataset), paths
            ):
                pbar.update()
